...
int main(void) {
  ...
  if((cfPtr = fopen("nameage.dat", "wb"))==NULL){
    ...
    rewind(cfPtr);
    while(fread(individual ,sizeof(struct student), 1, cfPtr)){
      for(i=0; i<20; ++i){
	//name & major match 
	if(strcmp(lname, individual[i].lastName) == 0 && strcmp(qMajor, individual[i].major)== 0){
	  printf("%s", individual[i].lastName);
	  printf("%s", individual[i].firstName);
	  printf("%s", individual[i].major);
	  printf("%f", individual[i].gpa);
	}
                
	//no match above name match
	else if(strcmp(lname, individual[i].lastName) == 0){
	  printf("%s %s %s %f", individual[i].firstName, individual[i].lastName, individual[i].major, individual[i].gpa);
	}
	// still no match, string match 
	else if(strcmp(qMajor, individual[i].major) == 0){
	  printf("%s %s %s %f", individual[i].firstName, individual[i].lastName, individual[i].major, individual[i].gpa);
	}
	...
	//no matches at all 
      ...
      /* int j;
            for(i= 0; i<sizeof(individual); ++i){
                for(i= 0; i<sizeof(individual); ++i){
                    if(strcmp(individual[i].firstName, individual[j].firstName) == 0 && strcmp(individual[i].lastName, individual[j].lastName) == 0 && strcmp(individual[i].major, individual[j].major)==0 && strcmp(individual[i].gpa, individual[j].gpa)==0){
