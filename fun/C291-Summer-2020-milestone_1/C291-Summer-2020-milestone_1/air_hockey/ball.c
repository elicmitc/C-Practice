...
bool checkCollisionWithGoal(ball_t *b, zone_t *z){

  if (b->upper_left_y<=z->upper_left_y || b->upper_left_y>=z->upper_left_y + z->width-20){
    b->speed_x *= -1;
    // b->speed_y *= -1;                                                                                                  

    player_1 +=1;
  }
  printf("Scores: Player 1: %d \t Player 2: ");
}
